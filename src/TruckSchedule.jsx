...
// Full merged TruckSchedule + Check-In/Out design & logic preserved
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { FiTruck, FiCalendar, FiFilter, FiSearch, FiCheckCircle, FiXCircle, FiRefreshCw } from 'react-icons/fi';
import { ToastContainer, toast } from 'react-toastify';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">Comprehensive fleet tracking and reporting</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)} className="w-full mt-1 px-4 py-2 border rounded" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">To Date</label>
              <input type="date" value={toDate} onChange={e => setToDate(e.target.value)} className="w-full mt-1 px-4 py-2 border rounded" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700">Truck Number</label>
              <input type="text" value={truckSearch} onChange={e => setTruckSearch(e.target.value)} className="w-full mt-1 px-4 py-2 border rounded" placeholder="Search truck..." />
            </div>
            <div className="flex items-end">
              <button onClick={() => fetchData(status, truckSearch)} className="w-full bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 flex items-center gap-2">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {['Dispatched', 'InTransit', 'CheckedOut', 'All'].map(btn => (
              <button key={btn} onClick={() => fetchData(btn, truckSearch)}
                className={`px-4 py-2 rounded text-sm ${status === btn ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}> 
                <FiCheckCircle className="inline mr-1" /> {btn}
              </button>
            ))}
          </div>

          <div className="mt-4">
            <button onClick={() => setShowPlantFilter(!showPlantFilter)} className="text-blue-600 flex items-center gap-1 text-sm">
              <FiFilter /> {showPlantFilter ? 'Hide Plants' : 'Show Plants'}
            </button>
          </div>
          ...
          {showPlantFilter && (
            <div className="mt-3 border rounded p-3 bg-gray-50">
              <div className="flex gap-2 mb-3">
                <button onClick={selectAll} className="bg-green-500 text-white px-3 py-1 rounded">Select All</button>
                <button onClick={deselectAll} className="bg-red-500 text-white px-3 py-1 rounded">Clear All</button>
              </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Slip</th>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Qty</th>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Freight</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.map((row, idx) => (
                    <tr key={idx} className="hover:bg-blue-50">
                      <td className="px-4 py-2 text-sm font-medium text-gray-900 uppercase">{row.truckNo || '—'}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{row.plantName || '—'}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{formatDateTime(row.checkInTime)}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{formatDateTime(row.checkOutTime)}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{row.loadingSlipNo || '—'}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{row.qty || '—'}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{row.freight || '—'}</td>
                      <td className="px-4 py-2 text-sm text-gray-700">{row.priority || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
